document.addEventListener("DOMContentLoaded", () => {
    ...
    // =========================================================================
    // 2. AUTOCOMPLETE DESAIN KUSTOM & SINKRONISASI INPUT ONE-WAY/ROUND-TRIP
    // =========================================================================
    ...
    const setupTravelAutocomplete = (input) => {
        // Mencegah setup berulang pada input yang sama
        if (input._hasAutocomplete) return;
        input._hasAutocomplete = true;

        const listContainer = document.createElement('ul');
        listContainer.className = 'custom-autocomplete-dropdown';
        
        // Desain CSS diletakkan di body secara absolut
        listContainer.style.cssText = `position:absolute;background:#fff;z-index:9999999;border:1px solid #ccc;border-radius:6px;max-height:280px;overflow-y:auto;box-shadow:0 8px 24px rgba(0,0,0,0.15);list-style:none;padding:0;margin:0;display:none;`;
        
        document.body.appendChild(listContainer);

        const updateDropdownPosition = () => {
            const rect = input.getBoundingClientRect();
            listContainer.style.top = (rect.bottom + window.scrollY + 5) + 'px'; // Jarak 5px dari input
        ...
        input.addEventListener('input', (e) => {
            ...
            }, 300); // 300ms delay untuk mengetik halus
        });

        // Memunculkan kembali container jika input difokuskan
        ...
        // Responsif saat window berubah ukuran/di-scroll
        ...
        // Menutup Dropdown ketika klik diluar elemen
